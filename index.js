/**
 * RDP Ultra Bot v6.0 - Production Grade
 * Developed by AlkshwlyHacker | 2026
 * Features: Unified Logger, Ephemeral Responses, Deferred Updates, !log command
 */
...
async function handleCommand(cmd, source, interaction = null) {
    ...
    if (source === 'discord' && interaction) {
        ...
        await sendTelegramMsg(
            response.replace(/\*\*(.+?)\*\*/g, '<b>$1</b>').replace(/`([^`]+)`/g, '<code>$1</code>'),
            2
        );
    }
    return response;
...
async function sendTelegramPanel() {
    ...
    try {
        ...
        const buttons = [
            ...
            [{ text: '📋 Log',        callback_data: 'btn_log'        }]
        ];

        if (tgPanelMsg) {
            try {
                await tgClient.editMessage(tgEntity, tgPanelMsg.id, undefined, text, { buttons, parseMode: 'html' });
            } catch (_) {
                tgPanelMsg = await tgClient.sendMessage(tgEntity, text, { buttons, parseMode: 'html' });
            }
        } else {
            tgPanelMsg = await tgClient.sendMessage(tgEntity, text, { buttons, parseMode: 'html' });
        }
...
async function checkTimeoutAlerts() {
    ...
    for (const t of ALERT_THRESHOLDS) {
        if (remaining <= t && !firedAlerts.has(t)) {
            ...
            if (WEBHOOK_URL) await sendDiscordWebhook('⏰ **TIMEOUT WARNING** - ' + mins + ' min remaining');
